...
printf "Google Oauth Client Id: ${GOOGLE_OAUTH_CLIENT_ID} \n"
printf "Google Oauth Client Secret: ${GOOGLE_OAUTH_CLIENT_SECRET} \n"
...
if  [ "${CORRECT}" == "y" ]; then
    ...
    sed -i '' s/__ADMIN_EMAIL__/"${ADMIN_EMAIL}"/g ${RUN_SCRIPT}
    sed -i '' s/__SITE_NAME__/"${SITE_NAME}"/g ${RUN_SCRIPT}
    sed -i '' s/__PORT__/"${PORT}"/g ${RUN_SCRIPT}
    sed -i '' s/__DB_DRIVER__/"${DB_DRIVER}"/g ${RUN_SCRIPT}
